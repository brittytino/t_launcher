...
class AppDrawerFragment : Fragment() {
    ...
    private fun initAdapter() {
        adapter = AppDrawerAdapter(
            ...
            appWhitelistListener = { appModel ->
                viewModel.toggleWhitelist(appModel.appPackage)
                requireContext().showToast("Toggled allowed status for ${appModel.appLabel}")
                viewModel.getAppList() // Refresh/Reload list to update icon state
            },
            appLimitListener = { appModel ->
                val options = arrayOf("15 minutes", "30 minutes", "45 minutes", "1 hour", "2 hours", "Remove Limit")
                android.app.AlertDialog.Builder(requireContext())
                    .setTitle("Set Daily Limit for ${appModel.appLabel}")
                    .setItems(options) { _, which ->
                        val limit = when (which) {
                            0 -> 15
                            1 -> 30
                            2 -> 45
                            3 -> 60
                        ...
                        if (limit > 0) {
                            viewModel.setAppLimit(appModel.appPackage, limit)
                        } else {
                            // TODO: Implement remove rule
                             requireContext().showToast("Limit removal not implemented yet")
                        }
                    }
                    .show()
            }
