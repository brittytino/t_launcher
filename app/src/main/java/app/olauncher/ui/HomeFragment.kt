...
class HomeFragment : Fragment(), View.OnClickListener, View.OnLongClickListener {
    ...
    private fun checkPermissions() {
        ...
        if (!requireContext().appUsagePermissionGranted()) {
            Toast.makeText(requireContext(), "T Launcher needs Usage Access for digital wellbeing features.", Toast.LENGTH_LONG).show()
            startActivity(Intent(android.provider.Settings.ACTION_USAGE_ACCESS_SETTINGS))
            return
        }

        // Check Accessibility Service Permission
        if (!app.olauncher.helper.isAccessServiceEnabled(requireContext())) {
            android.app.AlertDialog.Builder(requireContext())
                .setTitle("Enforcement Service Required")
                .setMessage("To enforce discipline rules, T Launcher needs its Accessibility Service enabled.")
        ...
        if (!isNotificationServiceEnabled()) {
             android.app.AlertDialog.Builder(requireContext())
                .setTitle("Notification Filter Required")
                .setMessage("To block distracting notifications, T Launcher needs Notification Access.")
                .setPositiveButton("Enable") { _, _ ->
